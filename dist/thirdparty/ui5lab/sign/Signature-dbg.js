...
sap.ui.define(["sap/ui/core/Control", "./library"], function (Control, library) {
  ...
  // d3's line generator
  var _createLine = d3.svg.line().interpolate("bundle").tension(1).x(function (d) {
    return d.x;
  }).y(function (d) {
    return d.y;
  });
  ...
  function _onStartDrawing() {
    ...
    _sPath = oSvg.append("path").data([_aPointData]).attr("fill", "none").attr("stroke-linejoin", "round").attr("stroke", this.getPenColor()).attr("stroke-width", this.getPenSize()).attr("d", _createLine);
  }
